...
describe("ProcessService", () => {
  ...
  describe("constructor", () => {
    ...
    it("should throw error for unsupported platform", () => {
      ...
      expect(() => new ProcessService()).toThrow("Unsupported platform");
  ...
  describe("findByPort", () => {
    it("should validate port number", async () => {
      ...
      await expect(processService.findByPort(-1)).rejects.toThrow(
        "Port must be between 1 and 65535"
      );
    });
    ...
    it("should throw error for non-numeric port", async () => {
      await expect(processService.findByPort(NaN)).rejects.toThrow(
        "Port must be a number"
      );
    });
  });
  ...
  describe("killProcess", () => {
    it("should throw error for invalid PID", async () => {
      await expect(processService.killProcess(0, false)).rejects.toThrow(
        "Invalid PID"
      );
      await expect(processService.killProcess(-1, false)).rejects.toThrow(
        "Invalid PID"
      );
    });
  ...
  describe("isCriticalProcess", () => {
    it("should detect critical system processes", () => {
      const criticalProcess: ProcessInfo = {
        pid: 1,
        port: 22,
        processName: "systemd",
    ...
    it("should not flag regular processes as critical", () => {
      const regularProcess: ProcessInfo = {
        pid: 12345,
        port: 3000,
        processName: "node",
