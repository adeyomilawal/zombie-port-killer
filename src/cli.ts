  ...
  .action(async (port: string | undefined, options) => {
    try {
      ...
      if (isNaN(portNum) || portNum < 1 || portNum > 65535) {
        ...
        console.error(
          chalk.gray('Port must be a number between 1 and 65535')
        );
        process.exit(1);
      }
  ...
  .action(async (action: string) => {
    try {
      switch (action.toLowerCase()) {
        ...
        default:
          console.error(
            chalk.red(`❌ Error: Invalid action "${action}"`)
          );
          console.error(
            chalk.gray(
              'Valid actions: enable, disable, check, status'
            )
          );
          process.exit(1);
      }
...
function showInfo(): void {
  ...
  if (isProject) {
    ...
    console.log(chalk.cyan('Type:        ') + chalk.white(projectType || 'Unknown'));
    console.log(chalk.cyan('Path:        ') + chalk.gray(projectPath));
    console.log(
      chalk.cyan('Common ports:') +
        chalk.white(` ${commonPorts.join(', ')}`)
    );
  } else {
    console.log(
