...
exports.SCAN_JSON_SCHEMA_VERSION = void 0;
exports.buildScanJsonDocument = buildScanJsonDocument;
exports.scanJsonV1ValidationErrors = scanJsonV1ValidationErrors;
exports.isValidScanJsonV1 = isValidScanJsonV1;
exports.SCAN_JSON_SCHEMA_VERSION = "1";
function toIsoStartTime(d) {
    if (!d || !(d instanceof Date) || isNaN(d.getTime())) {
        return null;
    }
    return d.toISOString();
}
function buildProcessContext(p, verbose) {
    if (!verbose) {
        return null;
    }
    return {
 ...
 */
function buildScanJsonDocument(input) {
    const { processes, verbose, zkillVersion, platform, filters, isCriticalProcess, getPortMapping, } = input;
    ...
    return {
        schemaVersion: exports.SCAN_JSON_SCHEMA_VERSION,
        meta: {
            zkillVersion,
            platform,
        },
...
const ALLOWED_SERVICE_MANAGERS = new Set(["systemd", "launchd", "windows-service"]);
function isNullableString(v) {
    return v === null || typeof v === "string";
}
function validateProcessContext(ctx, path, errors) {
    ...
    if (c.serviceManager !== null &&
        (typeof c.serviceManager !== "string" ||
            !ALLOWED_SERVICE_MANAGERS.has(c.serviceManager))) {
        errors.push(`${path}.serviceManager must be null or a known service manager string`);
    }
}
function validateProcessEntry(p, index, errors) {
 ...
 */
function scanJsonV1ValidationErrors(value) {
    const errors = [];
    ...
    const root = value;
    if (root.schemaVersion !== exports.SCAN_JSON_SCHEMA_VERSION) {
        errors.push(`schemaVersion must be "${exports.SCAN_JSON_SCHEMA_VERSION}"`);
    }
...
function isValidScanJsonV1(value) {
    return scanJsonV1ValidationErrors(value).length === 0;
}
//# sourceMappingURL=scan-json-schema.js.map