...
describe('ScanCommand', () => {
    ...
    beforeEach(() => {
        ...
        scanCommand = new scan_command_1.ScanCommand(mockProcessService, mockStorageService);
    ...
    describe('execute', () => {
        ...
        it('should show message when no ports are in use', async () => {
            ...
            await scanCommand.execute();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('No ports currently in use'));
        });
        it('should filter by port range', async () => {
            mockProcessService.getAllPorts.mockResolvedValue(mockProcesses);
            mockStorageService.getPortMapping.mockReturnValue(null);
            await scanCommand.execute({ range: '3000-6000' });
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Active Ports (2 found)'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Port range: 3000-6000'));
        });
        it('should filter by process name', async () => {
            mockProcessService.getAllPorts.mockResolvedValue(mockProcesses);
            mockStorageService.getPortMapping.mockReturnValue(null);
            await scanCommand.execute({ process: 'node' });
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Active Ports (1 found)'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Process: node'));
        });
        ...
        it('should hide system processes when system is false', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Active Ports (2 found)'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Hiding system processes'));
        });
        it('should filter by project name', async () => {
            ...
            expect(mockStorageService.getMappingsByProjectName).toHaveBeenCalledWith('my-project');
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Active Ports (1 found)'));
        });
        ...
        it('should sort ports by port number', async () => {
            ...
            const portCalls = calls.filter((call) => call[0].includes('Port'));
        ...
        it('should display verbose context when verbose option is true', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Running for:'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Started by:'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Working dir:'));
        });
        it('should not display verbose context when verbose option is false', async () => {
            ...
            expect(console.log).not.toHaveBeenCalledWith(expect.stringContaining('Running for:'));
            expect(console.log).not.toHaveBeenCalledWith(expect.stringContaining('Working dir:'));
        });
        it('should display service manager information when available', async () => {
            const processWithService = {
                pid: 1234,
                port: 3000,
                processName: 'nginx',
                command: 'nginx -g daemon off;',
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('nginx.service'));
        });
        it('should format uptime correctly for different durations', async () => {
            ...
            await scanCommand.execute({ verbose: true });
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Running for:'));
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('2d'));
        });
        it('should throw error for invalid port range format', async () => {
            mockProcessService.getAllPorts.mockResolvedValue(mockProcesses);
            await expect(scanCommand.execute({ range: '3000' })).rejects.toThrow('Invalid port range format');
        });
        it('should throw error for non-numeric port range', async () => {
            mockProcessService.getAllPorts.mockResolvedValue(mockProcesses);
            await expect(scanCommand.execute({ range: 'abc-def' })).rejects.toThrow('Port range must contain valid numbers');
        });
    ...
    describe('listMappings', () => {
        it('should show message when no mappings exist', () => {
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('No port mappings configured'));
        });
        it('should display all mappings', () => {
            ...
            scanCommand.listMappings();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('Port Mappings (2 configured)'));
            ...
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('[auto-kill]'));
        ...
        it('should format dates correctly', () => {
            ...
            scanCommand.listMappings();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('minute'));
            mockStorageService.getAllMappings.mockReturnValue([
                {
                    port: 3000,
                    projectName: 'recent',
            ...
            scanCommand.listMappings();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('hour'));
            mockStorageService.getAllMappings.mockReturnValue([
                {
                    port: 3000,
                    projectName: 'recent',
            ...
            scanCommand.listMappings();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('day'));
        });
        it('should show "just now" for very recent dates', () => {
            ...
            scanCommand.listMappings();
            expect(console.log).toHaveBeenCalledWith(expect.stringContaining('just now'));
        });
    });
});
//# sourceMappingURL=scan.command.test.js.map