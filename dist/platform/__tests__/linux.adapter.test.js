...
const linux_adapter_1 = require("../linux.adapter");
...
describe('LinuxAdapter', () => {
    ...
    beforeEach(() => {
        adapter = new linux_adapter_1.LinuxAdapter();
        jest.clearAllMocks();
    });
    describe('findProcessByPort', () => {
        it('should use ss command when available', async () => {
            ...
            expect(mockExecSync).toHaveBeenCalledWith("ss -lptn 'sport = :3000'", { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] });
        ...
        it('should fallback to netstat when ss is not available', async () => {
            mockExecSync
                ...
                .mockReturnValueOnce('● node.service');
            const result = await adapter.findProcessByPort(3000);
            expect(mockExecSync).toHaveBeenCalledWith('netstat -ltnp 2>/dev/null | grep :3000', { encoding: 'utf-8', stdio: ['pipe', 'pipe', 'ignore'] });
            expect(result).not.toBeNull();
        });
